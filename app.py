...
import os, time, math, csv, io, traceback, threading, requests
...
API_KEY = os.environ.get("TWELVE_DATA_API_KEY", "demo")  # replace with your key
PAIRS = [p.strip() for p in os.environ.get("PAIRS", "EUR/USD,GBP/USD,USD/JPY,EUR/JPY,AUD/CAD").split(",") if p.strip()]
PRIMARY_INTERVAL = os.environ.get("PRIMARY_INTERVAL", "5min")
CONFIRM_INTERVAL = os.environ.get("CONFIRM_INTERVAL", "15min")
CONFIRM_ENABLE = CONFIRM_INTERVAL.lower() not in ("", "off", "none")
LOOKBACK = int(os.environ.get("LOOKBACK", "80"))
VALUE_AREA_PCT = float(os.environ.get("VALUE_AREA_PCT", "0.70"))
MIN_BODY_PCT = float(os.environ.get("MIN_BODY_PCT", "0.35"))
IST = ZoneInfo("Asia/Kolkata")

ENABLE_SCANNER = os.environ.get("ENABLE_SCANNER", "true").lower() in ("1","true","yes","on")
SCAN_DELAY_SEC = float(os.environ.get("SCAN_DELAY_SEC", "3"))
SCAN_INTERVAL_SEC = float(os.environ.get("SCAN_INTERVAL_SEC", "15"))

WEBHOOK_URL = os.environ.get("WEBHOOK_URL", "").strip() or None
TG_BOT_TOKEN = os.environ.get("TG_BOT_TOKEN", "").strip() or None
TG_CHAT_ID = os.environ.get("TG_CHAT_ID", "").strip() or None
...
def ist_now_str(): return ist_now().strftime("%Y-%m-%d %H:%M:%S %Z")

# ---------- Data ----------
def td_fetch(symbol, interval, outputsize):
    url = "https://api.twelvedata.com/time_series"
    params = {"symbol": symbol, "interval": interval, "outputsize": outputsize, "order": "ASC",
              "format": "JSON", "dp": 6, "apikey": API_KEY}
    r = requests.get(url, params=params, timeout=20)
    r.raise_for_status()
    data = r.json()
    if "values" not in data:
        raise RuntimeError(f"TwelveData {symbol} {interval}: {data.get('message') or data}")
    out = []
...
def build_volume_profile(symbol, candles, value_area_pct):
    ...
    if not bins: return {"bins":{}, "poc":None, "vah":None, "val":None}
    ...
    while cum < value_area_pct*total and (left>=0 or right<len(levels)):
        ...
        if rvol>=lvol:
            if right<len(levels): used.add(levels[right]); cum+=bins[levels[right]]; right+=1
        else:
            if left>=0: used.add(levels[left]); cum+=bins[levels[left]]; left-=1
...
def generate_signal(symbol, candles):
    ...
    emas=compute_emas(closes,(9,21,50))
    tf=trend_flag(emas)
    prof=build_volume_profile(symbol, candles[-LOOKBACK:], VALUE_AREA_PCT)
    ...
    out = {"signal":signal,"reason":"; ".join(reason) if reason else "No clear setup",
           "trend":tf,"poc":poc,"vah":vah,"val":val,"svg":candle_svg(last)}
    return out
    ...
    for sym in PAIRS:
        try:
            ...
            if res["signal"] in ("CALL","PUT"):
                send_webhook(sym, res)
                send_telegram(sym, res)
        except Exception as e:
...
def scanner_loop():
    while ENABLE_SCANNER:
        ...
        try: scan_once()
        except Exception as e: STATE["errors"].append(f"scan: {e}")
...
def analyze():
    try:
        res = scan_once()
        return jsonify({"ok":True, "time_ist": STATE["last_scan_ist"], "results": res})
    except Exception as e:
...
@app.route("/health")
def health():
    return jsonify({"ok":True,"scanner":ENABLE_SCANNER,"last_scan_ist":STATE["last_scan_ist"],"errors":list(STATE["errors"])} )
